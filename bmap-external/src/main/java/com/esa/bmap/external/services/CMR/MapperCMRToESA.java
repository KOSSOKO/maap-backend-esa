...
public class MapperCMRToESA implements CMRAdditionAttributes {
	...
	@Value("${nasa.s3.http.endpoint}")
	private String nasaHttpEndpoint;
	...
	public com.esa.bmap.model.Granule mapCMRToESA(Granule granule, Collection collection) throws BmapException {
		...
		if (bmaapGranule.getProductType() != null
				&& bmaapGranule.getProductType().equalsIgnoreCase(com.esa.bmap.model.Granule.PRODUCT_TYPE_ROI)
				&& !Utils.isNasaGranule(granule)) {
	 ...
	 * 
	 * @param cmrGranule the CMR object
	 * @param bmaapGranule the bmaap object
	 * @return the bmaap object
	 * @throws BmapException
	 */
	private com.esa.bmap.model.Granule createData(Granule cmrGranule, com.esa.bmap.model.Granule bmaapGranule)
			throws BmapException {
		if (cmrGranule.getOnlineAccessURLs() != null && cmrGranule.getOnlineAccessURLs().getOnlineAccessURL() != null
				&& cmrGranule.getOnlineAccessURLs().getOnlineAccessURL().size() > 0) {
			LOG.info("createData for = " + cmrGranule.getGranuleUR());

			List<String> geometryTypeValues = additionalAttributesTypeList.get(ADDITIONAL_ATTRIBTE_GEOMETRY_TYPE);
			for (int i = 0; i < cmrGranule.getOnlineAccessURLs().getOnlineAccessURL().size(); i++) {
				String urlOnLine = cmrGranule.getOnlineAccessURLs().getOnlineAccessURL().get(i).getURL();
				URL aURL = null;
				try {
				...
				data.setUploadDate(convertCMRDateToESADate(cmrGranule.getLastUpdate()));
				data.setFilePath(localFilePath);
				data.setUrlToData(cmrGranule.getOnlineAccessURLs().getOnlineAccessURL().get(i).getURL().replace("s3://",
						nasaHttpEndpoint));

				// parse Double to float and set size

				addLayerName(cmrGranule, bmaapGranule, data);

				bmaapGranule.getDataList().add(data);
	 ...
	 * @throws BmapException
	 */
	private void addLayerName(Granule cmrGranule, com.esa.bmap.model.Granule bmaapGranule, Data data)
			throws BmapException {

		Map<String, List<String>> additionnalAttributesMap = getMapOfAdditionalAttributes(cmrGranule);
		// Set Product Type if specified
		List<String> values = additionalAttributesTypeList.get(ADDITIONAL_ATTRIBUTE_PRODUCT_TYPE);
		String productType = null;
		if (values != null && values.size() > 0) {
			productType = values.get(0);
		}

		if (cmrGranule.getOnlineAccessURLs().getOnlineAccessURL().size() == 1) {
			if (cmrGranule.getDataGranule() != null && cmrGranule.getDataGranule().getSizeMBDataGranule() != null) {
				data.setFileSize((float) (cmrGranule.getDataGranule().getSizeMBDataGranule() * SIZE_FACTOR_1000));
			}
		} // else we can't as sign file size on many online access url
		data.setAcquisitionDate(convertCMRDateToESADate(cmrGranule.getLastUpdate()));
		...
		if (productType != null && productType.equalsIgnoreCase(com.esa.bmap.model.Granule.PRODUCT_TYPE_SLC)) {
			fileWithoutExtension = fileWithoutExtension + "_amplitude";
		}

		// setting layer name of granule
		if (Utils.isNasaGranule(cmrGranule)) {
			try {
				data.setLayerName(Utils.getLayerNameFromGranule(cmrGranule));
			} catch (UnsupportedEncodingException e) {
				throw new BmapException("Failed to retrieve ");
			}
		...
		// setting carto server url if exists
	...
	public com.esa.bmap.model.Granule mapToBMAAGranule(Granule granule, Collection collection,
		...
		// adding spatial attributes to bmaapGranule
		if (granule.getSpatial() != null) {
			addSpacialAttributes(granule, bmaapGranule);
		}

		// adding subRegion if exists to bmaapGranule
		String subRegion = checkGranuleContainsSubRegion(granule);
		if (subRegion != null) {
			bmaapGranule.setSubRegion(new SubRegion(subRegion));
		}
	...
	private Instrument createBMAAPInstrument(InstrumentRef instrumentRef) {
		...
		if (instrumentRef.getSensors() != null) {
			for (SensorRef sensorRef : instrumentRef.getSensors().getSensor()) {
				...
				// checking if sensor list is null
				if (instrument.getSensors() == null) {
					instrument.setSensors(new ArrayList<Sensor>());
	 ...
	 */
	private String checkGranuleContainsSubRegion(Granule granule) {
		LOG.debug("Start of checkGranuleContainsSubRegion Class");

		String subRegion = null;
		List additionalAttributesList = granule.getAdditionalAttributes().getAdditionalAttribute();
		for (Iterator iterator = additionalAttributesList.iterator(); iterator.hasNext();) {
			AdditionalAttributeRef object = (AdditionalAttributeRef) iterator.next();

			if (object.getName().equalsIgnoreCase("Site Name")) {
				subRegion = object.getValues().getValue().get(0);
				break;
			}
		}
