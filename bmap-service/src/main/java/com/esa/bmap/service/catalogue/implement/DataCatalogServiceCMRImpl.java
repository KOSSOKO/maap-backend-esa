...
public class DataCatalogServiceCMRImpl implements
		DataCatalogServiceCMRInterface {
	private static final Logger LOG = LoggerFactory
			.getLogger(DataCatalogServiceCMRImpl.class);
	 ...
	 */
	@Override
	public Granule addData(Granule granule) throws BmapException {
	 ...
	 */
	@Override
	public Collection<Granule> getDataByCriteria(GranuleCriteria dataCriteria)
			throws BmapException {
		Collection<Granule> dataCollection = dataRepositoryCMR
				.findByCriteria(dataCriteria);
	...
	public boolean testConnectionTimeOut(int timeOut, boolean doTest) {
		...
		if (doTest) {
			...
			try {
				...
				uneURL = new URL(
						Common.getPropertiesValue("Request.Adresses.Base"));
